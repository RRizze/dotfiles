...
function helpers.update_taglist(text_widget, tag, index, container)
  if tag.selected then
    text_widget.markup = helpers.colorize_text(beautiful.taglist_icons[index], beautiful.taglist_text_color_focused)
    container.color = beautiful.taglist_text_color_focused
  elseif tag.urgent then
    text_widget.markup = helpers.colorize_text(beautiful.taglist_icons[index], beautiful.taglist_text_color_urgent)
    container.color = beautiful.bg_normal
  elseif #tag:clients() > 0 then
    text_widget.markup = helpers.colorize_text(beautiful.taglist_icons[index], beautiful.taglist_text_color_occupied)
    container.color = beautiful.bg_normal
  else
    text_widget.markup = helpers.colorize_text(beautiful.taglist_icons[index], beautiful.taglist_text_color_empty)
    container.color = beautiful.bg_normal
   end
end
